...
class Neo4jConnector:
    ...
    def get_shortest_path_by_cost(self, origen, destino, medio_transporte):
        """
        Encuentra la ruta más barata entre dos lugares usando un medio de transporte.
        Retorna la ruta como una lista de nombres de lugares y el costo total.
        """
        cost_property = medio_transporte # 'costo_bus' o 'costo_avion'
        query = f"""
        MATCH p=(start:LUGAR)-[rels:CONEXION*]->(end:LUGAR)
        WHERE start.nombre = $origen AND end.nombre = $destino
        WITH p, reduce(cost = 0, r IN rels | cost + r.{cost_property}) AS total_cost
        RETURN [n IN nodes(p) | n.nombre] AS path_names, total_cost
        ORDER BY total_cost ASC
        LIMIT 1
        """
        with self.driver.session() as session:
            result = session.run(query, origen=origen, destino=destino).single()
            if result:
                return result['path_names'], result['total_cost']
        return None, None

    def get_shortest_path_by_distance(self, origen, destino):
        """
        Encuentra la ruta más corta en distancia entre dos lugares.
        Retorna la ruta como una lista de nombres de lugares y la distancia total.
        """
        query = """
        MATCH p=(start:LUGAR)-[rels:CONEXION*]->(end:LUGAR)
        WHERE start.nombre = $origen AND end.nombre = $destino
        WITH p, reduce(dist = 0, r IN rels | dist + r.distancia) AS total_distance
        RETURN [n IN nodes(p) | n.nombre] AS path_names, total_distance
        ...
        with self.driver.session() as session:
            result = session.run(query, origen=origen, destino=destino).single()
            if result:
                return result['path_names'], result['total_distance']
        return None, None